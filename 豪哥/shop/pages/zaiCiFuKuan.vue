<template>
	<view>
		...
		<view class="list">
			<view class="item" v-for="item in order.items">
				...
				<view class="xiangqing">
					...
					<view class="chima">{{item.productAttribute.color}}，{{item.productAttribute.size}}</view>
					...
					<view class="shijijiage">
						单价：{{order.payType==2 ? '￥'+item.price : item.price+'积分'}}
					</view>
				</view>
				<view class="shuliang">
					x{{item.count}}
		...
		<uni-notice-bar text="提示:订单已生成,备注,以及重新选择地址"></uni-notice-bar>
