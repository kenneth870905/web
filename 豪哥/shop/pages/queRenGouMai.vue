<template>
	<view>
		<view class="diZhi" @click="收货地址()">
			<text :class="{active:!shdz.id}">{{shdz.id ? shdz.address+"("+shdz.class+")" : '请选择收货地址'}}</text>
			<icon class="icon iconfont iconarrow-right"></icon>
		</view>
		
		<view class="list">
			<view class="item" v-for="item in spList">
				<image :src="api_url+'/'+item.sp.coverImg" mode="aspectFill"></image>
				<view class="xiangqing">
					...
					<!-- <view class="jiage">
						<text v-if="!isMiaosha">￥{{item.danjia}}</text>
		...
		<view class="box2">
			<view class="item2">
				<text>备注</text>
				<input type="text" v-model="comment" placeholder="留言或建议" />
			</view>
	...
	export default {
		data() {
			return {
				...
				// arr:[{payType:2,rangeKey:"微信支付"},{payType:1,rangeKey:"积分支付"}],
				arr:[{payType:2,rangeKey:"微信支付"}],
				zhifufangshi:{payType:2,rangeKey:"微信支付"},
				comment:"",		//备注
				orderid:"",
			};
		},
		...
		methods:{
			...
			提交订单(){
				...
				// }
				var obj ={
						"payType": this.zhifufangshi.payType, 		//支付方式,积分支付1，微信支付2
						"items": [],			//商品列表
						"comment":this.comment,
						addressId: this.shdz.id , //收货地址Id
					}
				this.spList.forEach(item=>{
					obj.items.push({productId:item.sp.id,count:item.number,productAttributeId:item.guigeId})
				})
				uni.showLoading({
					title: '加载中',
					mask: true
			...
			支付接口(orderid){
				this.$http(`/api/order/${orderid}/pay`,'',"POST").then(x=>{
					console.log(x)
					if(x.code===0){
						var data= x.data
						uni.requestPayment({
		...
		mounted() {
			...
			for (var i= 0;i<this.spList.length;i++) {
				let danjia = this.spList[i].sp.productAttributes.find(x=>x.id==this.spList[i].guigeId).price
					this.$set(this.spList[i],'danjia',danjia)
