<template>
    <div>
        <div ref="锚点">
            <el-page-header  @back="goBack" :content="sp.id ? '编辑秒杀商品' : '添加秒杀商品'"></el-page-header>
        </div>
        <div class="form">
            <el-form :model="sp" :rules="spRules" ref="spFrom" size="small" label-width="80px" @submit.native.prevent>
                ...
                <el-form-item label="秒杀价格" >
                    <el-input v-model="sp.seckillPrice" type="number" step="0.01" @change="change_1('seckillPrice')" placeholder="请输入秒杀价格"></el-input>
                </el-form-item>
...
export default {
    ...
    methods: {
        ...
        getproduct(){
            ...
            .then(res => {
                if(res.code==0)
                console.log(res.data.seckillPrice)
                this.sp = res.data
            })
            .catch(err => {
            })
