    ...
    <div>
        ...
        <div class="form">
            <el-form :model="sp" :rules="spRules" ref="spFrom" size="small" label-width="80px" @submit.native.prevent>
                ...
                <el-form-item label="标价" prop="levelOnePrice">
                    <el-input v-model="sp.levelOnePrice" type="number" step="0.01" @change="change_1('levelOnePrice')" placeholder="请输入价格"></el-input>
                </el-form-item>

                <el-form-item label="规格">
                    <template>
                        <div>
                            <el-button @click="添加规格()" type="warning" size="mini">添加</el-button>
                        </div>
                        <el-table :data="sp.productAttributes" border="" size="mini">
                            <el-table-column label="规格">
                                <template slot-scope="s">
                                    <el-input v-model="s.row.color" placeholder=""></el-input>
                                </template>
                            </el-table-column>
                            <el-table-column label="小图">
                                <template slot-scope="s">
                                    <div class="xiaotu">
                                        <i @click="changeFile(4,s.row)" v-if="!s.row.colorImg" class="el-icon-plus"></i>
                                        <i v-if="s.row.colorImg" class="el-icon-delete" @click="s.row.colorImg=''"></i>
                                        <img @click="changeFile(4,s.row)" v-if="s.row.colorImg" :src="$api_url+'/'+s.row.colorImg" alt="">
                                    </div>
                                </template>
                            </el-table-column>
                            <el-table-column label="尺寸">
                                <template slot-scope="s">
                                    <el-input v-model="s.row.size" placeholder=""></el-input>
                                </template>
                            </el-table-column>
                            <el-table-column label="库存">
                                <template slot-scope="s">
                                    <el-input v-model.number="s.row.count" placeholder=""></el-input>
                                </template>
                            </el-table-column>
                            <el-table-column label="实际价格">
                                <template slot-scope="s">
                                    <el-input v-model.number="s.row.price" type="number" step="0.01" placeholder=""></el-input>
                                </template>
                            </el-table-column>
                            <el-table-column width="100px" align="center">
                                <template slot-scope="s" >
                                    <el-button type="warning" size="mini" @click="sp.productAttributes.splice(s.$index,1)">删除</el-button>
                                </template>
                            </el-table-column>
                        </el-table>
                    </template>
...
export default {
    ...
    data() {
        return {
            ...
            showFile: false,
            file_type: 1, //1封面 2 轮播图 3 富文本图片 4 规格小图
            规格:{},    //选中规格
    ...
    methods: {
        添加规格(){
            this.sp.productAttributes.push({
                "color": "",
                "colorImg": "",		
                "count": 0,				//库存
                "size": "",				 //尺寸
        ...
        修改(){
            ...
            Promise.all([this.upload1(),this.upload2()]).then(list=>{
                ...
                this.sp.productAttributes.forEach(item=>{
        ...
        change_1(key){
            this.sp[key] = this.sp[key] ? parseFloat(Math.ceil(this.sp[key]*100)/100)  : 0
        },
        获取商品分类(){
        ...
        // 获取规格(){
        //     this.$axios.post('/api/productAttribute',{productId: parseInt(this.sp.id) })
        //     .then(res => {
        //         console.log(res)
        //     })
        //     .catch(err => {
