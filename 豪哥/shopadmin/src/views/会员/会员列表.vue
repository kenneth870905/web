<template>
    <div>
        ...
        <div class="header1">
            <el-input placeholder="请输入会员昵称" class="搜索" v-model="query.name" size="mini">
                <!-- <el-select v-model="select" slot="prepend" placeholder="请选择">
                    <el-option label="餐厅名" value="1"></el-option>
                    <el-option label="订单号" value="2"></el-option>
                    <el-option label="用户电话" value="3"></el-option>
                </el-select> -->
                <el-button slot="append" @click="搜索()" icon="el-icon-search"></el-button>
            </el-input>
        ...
        <el-dialog title="积分统计" class="全部统计" :visible.sync="显示全部积分" width="800px">
            <div class="积分统计">
                <div class="title2">
                    <div>选择日期查询</div>
                    <div>
                        <el-date-picker value-format="yyyy-MM-dd" @change="changeTime3" size="small" v-model="time3" type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期"></el-date-picker>
                        <el-button type="" @click="下载()" size="small" style="margin-left:10px;">下载</el-button>
                    </div>
                </div>
                <div class="table2">
                ...
                <div class="分页">
                    <el-pagination background @current-change="changePage3" :page-size="query.size" layout="prev, pager, next" :total="total3"></el-pagination>
                </div>
            </div>
        </el-dialog>
    </div>
...
export default {
    ...
    methods: {
        下载(){
            ...
            this.$axios.get('/api/export/creditLog',{params:q})
            .then(res => {
                if(res.code===0){
