...
export default {
    ...
    methods: {
        ...
        获取分类() {
            this.$axios.get('/api/articleCategory', { params: this.query }).then(res => {
                this.list = res.data
            }).catch(err => {
            })
        },
        ...
        async 确定() {
            ...
            if (!this.分类.id) {
                this.$axios.post('/api/articleCategory', { name: this.分类.name,img:this.分类.img }).then(res => {
                    if (res.code === 0) {
                        this.正确('添加成功');
                        this.显示详情 = false
                        this.获取分类()
                ...
                this.$axios.put(`/api/articleCategory/${this.分类.id}`, { name: this.分类.name,img:this.分类.img }).then(res => {
                    if (res.code === 0) {
                        this.正确('修改成功');
                        this.显示详情 = false
                        this.获取分类()
        ...
        删除(item) {
            this.$confirm('确定删除？?', '提示', {
                ...
                this.$axios.delete(`/api/articleCategory/${item.id}`)
                .then(res => {
                    if(res.code===0){
                        this.正确('删除成功')
                        this.获取分类()
