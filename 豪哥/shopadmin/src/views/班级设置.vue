<template>
    <div>
        <div class="title1">
            ...
            <el-button type size="mini" @click="添加()">添加班级</el-button>
        </div>
        ...
        <el-dialog class="分类dialog" :close-on-click-modal="false" :title="分类.id ? '班级信息' : '添加班级'" :visible.sync="显示详情" width="300px">
            <div>
                <div class="title">班级名称</div>
                <el-input @keydown.enter.native="确定()" v-model="分类.name" placeholder="请输入班级名称"></el-input>
...
export default {
    ...
    methods: {
        ...
        获取分类() {
            this.$axios.get('/api/class', { params: this.query }).then(res => {
                this.list = res.data
            }).catch(err => {
            })
        },
        ...
        async 确定() {
            ...
            if (!this.分类.id) {
                this.$axios.post('/api/class', { name: this.分类.name}).then(res => {
                    if (res.code === 0) {
                        this.正确('添加成功');
                        this.显示详情 = false
                        this.获取分类()
                ...
                this.$axios.put(`/api/class/${this.分类.id}`, { name: this.分类.name,img:this.分类.img }).then(res => {
                    if (res.code === 0) {
                        this.正确('修改成功');
                        this.显示详情 = false
                        this.获取分类()
        ...
        删除(item) {
            this.$confirm('确定删除？?', '提示', {
                ...
                this.$axios.delete(`/api/class/${item.id}`)
                .then(res => {
                    if(res.code===0){
                        this.正确('删除成功')
                        this.获取分类()
