<template>
	<view class="easy-select" @click.stop="trigger" :style="[easySelectSize]">
		<input type="text" v-model="value.label" :placeholder="placeholder" disabled clearable>
		...
		<!-- <view class="easy-select-options" v-if="showOptions" :style="{'min-width': boundingClientRect.width + 'px', top: optionsGroupTop}"> -->
		...
		<view v-if="showOptions" class="bg">
			背景
		</view>
	</view>
</template>
...
<script>
	...
	const MAX_OPTIONS_HEIGHT = 250 // 修改务必也修改easy-select-options的css部分
			...
			trigger(e) {
				...
				view.select('.easy-select').fields({rect: true}, data => {
					...
					if (bottom < thresholdHeight) {
						...
						this.optionsGroupTop = -thresholdHeight  + 'px'
