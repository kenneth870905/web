<template>
    <div>
        ...
        <!-- <el-table v-loading="loading" :data="list" stripe border size="mini" @sort-change="change1"> -->
            ...
            <el-table-column fixed label="会员id" prop="uid" width="60px" align="center">
                <template slot-scope="s">
                    <userPopover :userId="s.row.id">
                        <el-button type="text">{{s.row.id}}</el-button>
                    </userPopover>   
                </template>
            </el-table-column>
            <el-table-column label="金额" prop="amount" width="120px"></el-table-column>
            <el-table-column label="收费(charges)" prop="charges"></el-table-column>
            <el-table-column label="订单(cid)" prop="cid"></el-table-column>
            <el-table-column label="审核(reviewer)" prop="reviewer"></el-table-column>
            <el-table-column label="审核时间(reviewed_at)" prop="reviewed_at" width="160px">
                <template slot-scope="s">
                    <div>{{s.row.reviewed_at}}</div>
                    <div class="bjTime">{{s.row.reviewed_at | bjTime}}</div>
                </template>
            </el-table-column>
            ...
            <el-table-column label="备注(remarks)" prop="remarks"></el-table-column>
            <el-table-column label="出款(withdrawer)" prop="withdrawer"></el-table-column>
            <el-table-column label="出款时间(withdrawed_at)" prop="withdrawed_at"></el-table-column>
...
export default {
    ...
    methods: {
        ...
        getList(){
            this.loading = true
            this.$axios.get('WithdrawRecords',{params:this.query}).then(res => {
                if(res.result){
                    this.list = res.data
