<template>
    <div class="flex100">
        ...
        <el-table class="table" height="100%" v-loading="loading" :data="list" stripe border size="mini" >
            ...
            <el-table-column label="操作员" prop="nick"></el-table-column>
            <el-table-column label="Oid" prop="oid">
                <template slot-scope="s">
                    <el-button type="text" v-if="s.row.cid==1" @click="$router.push('/Details?id='+s.row.oid)">{{s.row.oid}}</el-button>
                    <span v-if="s.row.cid==2 || s.row.cid==3">{{s.row.oid}}</span>
                </template>
            </el-table-column>
...
export default {
    ...
    methods: {
        ...
        getList() {
            this.loading=true
            this.$axios.post('Log',this.query ).then(res => {
                if (res.result) {
                    this.list = res.data
