<template>
    <div class="address">
        ...
        <div class="form">
            ...
            <div style="margin: 16px;" v-if="address.id">
                <van-button @click="删除()" round block type="warning">删除</van-button>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        // this.$toast.clear();
                this.$axios.post('/Address/delete',{id:this.address.id}).then(res => {
                    console.log(res)
                    if(res.code==1){
                        this.$toast('删除成功')
                        setTimeout(() => {
