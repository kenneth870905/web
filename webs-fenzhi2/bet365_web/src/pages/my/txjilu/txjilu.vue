    ...
    <div>
        ...
        <div class="mui-content mui-fullscreen">
            ...
            <div class="content_1" ref="content1" @scroll="scroll($event)">
                <van-list
                    v-model="数据.loading"
                    :finished="!数据.loading && 数据.total<=数据.list.length"
                    finished-text="没有更多了"
                    @load="xiayiye"
                >
                        ...
                        >
                            <div class="mui-table">
                                <div class="mui-table-cell mui-col-xs-7">
                                    <h4
                                        class="mui-ellipsis"
                                        :class="{'蓝色字体':item.Code}"
                                    >({{item.SystemType}}){{item.Type}}</h4>
                                    <p class="mui-h6">{{item.Note}}</p>
                                </div>
                                <div class="mui-table-cell mui-col-xs-3 mui-text-right">
                                    <span
                                        class="mui-h5"
                                        :class="getAmountColor(item)"
                                    >{{item.Amount}}</span> 元
