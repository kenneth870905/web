<template>
    <div class="走势内容">
        <ul class="title_1">
            <li :class="{'active':走势查看==0}" @tap="走势查看=0">百位走势</li>
            <li :class="{'active':走势查看==1}" @tap="走势查看=1">十位走势</li>
            <li :class="{'active':走势查看==2}" @tap="走势查看=2">个位走势</li>
        </ul>
        <ul class="走势列表">
            <li class="header">
                <div class="期数">期号</div>
            ...
            <li v-for="(item, index) in 走势号码" :key="index">
                ...
                <div class="号码">
                    ...
                    <div class="item" :class="{'active':item.number==1}"> <span>0</span> </div>
                ...
                for (let i = 0; i < list.length; i++) {
                    if(list.length-1>i){
                        if(list[i].number < list[i+1].number){
                            list[i].start = 'left';
                            list[i].left = (list[i].number+1)*this.width - this.width/2;
                        }else{
                            list[i].start = 'right';
                            list[i].left = (list[i+1].number+1)*this.width - this.width/2;
                        }
