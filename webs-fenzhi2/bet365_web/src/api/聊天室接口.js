...
const api_投注跟单=(obj)=>{
    return axios.post(config.api_url+'/Mapi/Chat/ChatOrderBuy?orderId='+obj.Id+'&amount='+obj.amount)
}
...
const api_存取汇总=(obj)=>{
    return axios.post(config.api_url+'/mapi/Chat/GetMoneyTransaction',qs.stringify(obj));
}

const api_最新发言记录=(obj)=>{
    return axios.post(config.api_url+'/mapi/Chat/GetLatestMessages',qs.stringify(obj));
}

const api_用户是否禁言=(obj)=>{
    return axios.post(config.api_url+'/mapi/Chat/IsMute',qs.stringify(obj))
}
