...
<div class="video">
             <div class="top">
                <div v-if="!搜索后显示列表" class="icon" @click="返回()"><van-icon name="arrow-left" /></div>
                <div v-if="搜索后显示列表" class="lei" @click="分类列表() ">分类列表</div>
                <div class="c_c"> 
                ...
                <div class="video_content" v-if="搜索后显示列表"> 
                    <ul class="guanz_ul">
                        <li @click="查看小说(keey)" v-for="(keey,index) in objSS.list">
                ...
                <div class="video_content"   v-show="!搜索后显示列表">
                    <van-tabs  @click="onClick"  animated>
                        <van-tab v-for="(item,index) in title" :title="item">
                            <van-list class="ul外容器" v-model="loading" :offset="20" :finished="finished" :finished-text="objdata.total==0 ? '暂无数据' : '没有更多了'" @load="onLoad">
                                <ul class="guanz_ul">
                                    <li @click="查看小说(keey)" v-for="(keey,index) in objdata.list">
                                        <div class="img_box" ><img v-on:error.once="dosomething($event)" :src="configData.api_url+keey.fengmian" alt=""></div>
                                        <div class="name_title">{{keey.name}}</div>
                                        <div class="text_bottom">
                                            <div class="收藏 fac">{{keey.last_update_chapter_name}}</div>
...
<script>
import {小说大分类,小说大分类对应的内容,小说详情页,小说搜索} from "@/api/小说.js"
