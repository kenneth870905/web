    ...
    <div class="专家">
             ...
             <van-tabs v-model="关注active" animated>
                <van-tab v-for="(item,index) in 关注" :title="item.xingx==0 ?item.title:item.title+'(' +item.xingx+')'"  >
                    <ul class="ul_c" >
                        ...
                        <li   v-show="关注active==0"   v-for="(item,index) in 关注[0].minlist"  class="关注li"   @click="查看专家详情(item)" >
                            <div class="top_z">
                                <div class="left">
                                    <div class="div_box"><img src="@/assets/img/ty.png" alt=""></div>
                                </div>
                                <div class="right">
                                    <div class="l_r">
                                        <p class="last_child"><span class="name">{{item.name}}</span><span class="sf">{{item.type}}</span></p>
        ...
        <div  class="足球" v-if="type==1">
               <van-tabs v-model="足球active" animated  @click="onClick">
                <van-tab v-for="(value,key,index) in 足球list" :title="value.title"  >
                    <ul class="ul_c" >
                        <li class="足球li"   v-for="(items,indexmin) in  value.minlist "  @click="查看专家详情(items)" >
                             <div class="top_z">
                                <div class="left" >
                                ...
                                <!-- <div class="right" @click="$router.push('/ExpertsDetails')"> -->
                                <div class="right">
                                    <div class="l_r">
                                        <p class="last_child"><span class="name">{{items.name}}</span><span class="sf" v-show="足球active==2">篮球评论员</span></p>
                                        <div class="pl" v-show="足球active==0 || 足球active==1">
                                            <span>{{items.type}}</span>
                                            <div class="jing_l">{{items.ZuiJinZhanJi}}</div>
                                            <div class="l_h" v-if="items.lianhong!==0">{{items.lianhong}}连红</div>
                                        </div>
                                        <p class="pl" v-show="足球active==2">近期命中&nbsp; <span>{{items.MingZhongLv_avg}}%</span> </p>
                                        <p class="pl" v-show="足球active==0 || 足球active==2">平均培率&nbsp;<span>{{items.pingjunpeilv}}</span></p>
                                        <p class="pl" v-show="足球active==1">新增加粉丝&nbsp;<span>{{items.fans}}</span></p>
                                    </div>
                                    <div class="qi" v-show="足球active==0|| 足球active==1">
                                        <div class="nub_b">
                                            <div class="max_q" v-show="足球active==0 || 足球active==1">{{ items.yinglilv}}</div>
                                            <div class="bfb" v-show="足球active==0">%</div>
                                            <div class="hot" v-show="足球active==1">
                                                <van-icon name="fire" class="hot_s" />
                                            </div>
                                        </div>

                                        <div class="sh">{{足球active==0 ? "盈利率" : "周人气值"}}</div>
                                        <div class="bj" v-show="足球active==0 || 足球active==1  "><span class="iconfont iconjiangpai"></span></div>
                                    </div>
                                    <div class="r_r" v-show="足球active==2">
...
export default {
    ...
    methods: {
        ...
        onClick(name, title, item) {
            ...
            所有专家数据(obj).then(x => {
                console.log(x)
                this.足球list[name].minlist = x.data.data.list
            }).catch(err => {
                console.log(err)
            })
        },
