<template>
    <div class="专家">
        ...
        <div class="关注" v-if="type==0">
            <van-tabs v-model="关注active" animated  @click="GZclick"> 
                <van-tab  title="关注的专家">
                    ...
                    <div v-else>
                        ...
                        <ul class="ul_c" v-else>
                            <van-list class="ul外容器" v-model="loading" :offset="20" :finished="finished" :finished-text="objdata.total==0 ? '暂无数据' : '没有更多了'" @load="onLoad">
                                <li  v-for="(item,index) in objdata.list" class="关注li" @click="查看专家详情(item,'gz')">
                                    <div class="top_z ">
                                        <div class="left">
                ...
                <van-tab title="关注的专家方案">
                   <div class="没有关注的内容" @click="$router.push('/login')"   v-if="!是否有登录">
                        <div class="img_box"> <img src="@/assets/img/wnr.png" alt=""></div>
                        <div class="提示文字">你还没有登录,请点击进行登录</div>
                    </div>
                    <div v-else>
                        <div class="没有关注的内容"  v-if="关注专家方案show">
                            <div class="img_box"> <img src="@/assets/img/wnr.png" alt=""></div>
                            <div class="提示文字"> 您关注的专家暂无方案 </div>
                        </div>

                        <ul class="ul_c" v-else>
                            <li class="关注的专家方案" v-for="(item,index) in  关注的专家方案数据" @click="查看方案详情(item )">
                                <div class="top">
...
export default {
    ...
    methods: {
        ...
        关注的专家方案函数() {
            关注的专家方案().then(x => {
                if(x.data.data.listlength==0){
                  this.关注专家方案show=true
                }else{
                    this.关注专家方案show=false
        ...
        GZclick(index, title, item){
            ...
            if(index==1){
                if(!this.关注专家show){
                 ...
                 this.objdata= {  list: [],  total: -1    }
                 this.currentPagerm=0 //当前页数 
                 this.onLoad()
            }
        },
