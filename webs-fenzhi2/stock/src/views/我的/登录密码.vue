<template>
    <div>
        <van-nav-bar title="登录密码" :z-index="10" left-arrow @click-left="$back()"/>

        <div class="content">
            <van-cell-group>
                      <van-field required v-model="obj.newPassword" label="新密码" :type="type.password2 ? 'password' : 'text'"  :right-icon="type.password2 ? 'closed-eye' : 'eye'"  @click-right-icon="type.password2=!type.password2" placeholder="请输入新的登录密码" />
                    <van-field required v-model="obj.confimPassword" label="确认新密码" :type="type.password3 ? 'password' : 'text'"  :right-icon="type.password3 ? 'closed-eye' : 'eye'" @click-right-icon="type.password3=!type.password3" placeholder="请再次确认登录密码" />
                  <van-cell-group>
                        <van-field
                            v-model="obj.mobile"
                            center
...
<script>
import { api_修改登陆密码,api_获取短息验证码} from "@/api/请求登录的接口.js";
...
export default {
    ...
    methods: {
          getSms() {//点击获取短信触发的函数
            ...
            api_获取短息验证码(this.obj.mobile).then(x => {
                if (x.data.code == 200) {
        ...
        提交(){
           if(!this.obj.newPassword){
            ...
            var obj={
                ...
                newPwd :this.obj.confimPassword,
            }
            api_修改登陆密码(obj).then(x=>{
                if(x.data.code==200){
