                ...
                <div class="electron_C">
                    ...
                    <div class="AG_title">
                        <div class="title_img_left">
                            <img :class="activeName" :src="config.img_url+'image/dianziyouyi/'+activeName+'-s.png'" alt="" srcset="">
                        </div>
                        <div class="div_btn" @click="全部()">全部</div>
                        <div class="type_select" v-if="电子游艺[activeName]" v-show="电子游艺[activeName]['type'] && 电子游艺[activeName]['type'].length>0">
                            <el-dropdown @command="handleCommand">
                                <span class="el-dropdown-link">
                                    {{过滤类型.name}}<i class="el-icon-arrow-down el-icon--right"></i>
                                </span>
                                <el-dropdown-menu slot="dropdown">
                                    <el-dropdown-item :command="index" v-for="(item, index) in 电子游艺[activeName]['type']" :key="index">{{item}}</el-dropdown-item>
                                </el-dropdown-menu>
                            </el-dropdown>
                        </div>
                                ...
                                <ul>
                                    <li @click="进入游戏(item1)" v-for="(item1, index1) in 游戏列表" :key="index1">
                                        <div class="gameImg_box">
                                            <el-image v-loading="!item1.imgLoad" element-loading-background="rgba(189, 189, 189, 0.8)" @error="img_load(item1)" @load="img_load(item1)" :src="config.img_url+'image/dianziyouyi/'+activeName+'/'+item1.id+img_type" lazy></el-image>
                                            <!-- <img :src="config.img_url+'image/dianziyouyi/'+activeName+'/'+item1.id+img_type" alt=""> -->
                                            <div class="gouGame">
...
export default {
    ...
    computed: {
        // 页面一加载
        getMoney() { // 获取vuex的 平台余额字段
            var Money = this.$store.state.Money
            return Money
        },
        ...mapGetters({
            // 登陆信息:"登陆信息"
        ...
        img_type() {
            try {
                return this.电子游艺[this.activeName].img
            } catch (error) {
                console.log(error)
                return ''
            }
        ...
        游戏列表() {
            var list = (this.电子游艺[this.activeName] && this.电子游艺[this.activeName].game) ? this.电子游艺[this.activeName].game : []
    ...
    methods: {
        ...
        handleCommand(val) {
            this.过滤类型.type = val
            this.过滤类型.name = this.电子游艺[this.activeName].type[val]
        },
        全部() {
            this.过滤类型.type = 0
...
.electron-banner {
    background: url("~@/assets/image/dz-banner.jpg") no-repeat;
    width: 100%;
    padding-top: 440px;
}
