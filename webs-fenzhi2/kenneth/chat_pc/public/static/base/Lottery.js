...
Bridge.assembly("Lottery", function ($asm, globals) {
    ...
    Bridge.define("Lottery.BetHelper", {
        statics: {
            ...
            ctors: {
                init: function () {
                    this.ContentDaXiao = System.Array.init(["澶�", "灏�"], System.String);
                    this.ContentDanShuang = System.Array.init(["鍗�", "鍙�"], System.String);
                    this.ContentDaXiaoDanShuang = System.Array.init(["澶�", "灏�", "鍗�", "鍙�"], System.String);
            ...
            methods: {
                ...
                /**
                 * 鑾峰彇澶氳锛屾€婚」涓嶅彲閲嶅锛屾瘡涓€椤逛笉閲嶅鐨勫崟寮忔敞鏁�
                 *
                 * @static
                 * @public
                 * @this Lottery.BetHelper
                ...
                /**
                 * 鑾峰彇澶氳锛屾€婚」涓嶅彲閲嶅锛屾瘡涓€椤规帓搴忓悗涓嶉噸澶嶇殑鍗曞紡娉ㄦ暟
                 *
                 * @static
                 * @public
                 * @this Lottery.BetHelper
                ...
                /**
                 * 鑾峰彇锛屾€婚」涓嶅彲閲嶅锛屾瘡涓€椤瑰彲閲嶅鐨勫崟寮忔敞鏁�
                 *
                 * @static
                 * @public
                 * @this Lottery.BetHelper
                 ...
                 * @memberof Lottery.BetHelper
                 * @param   {Function}                           T       
                 * @param   {System.Collections.Generic.List}    list    杩斿洖鐨勮寖鍨�
                 * @param   {Array.<T>}                          t       鎵€姹傛暟缁�
                 * @param   {number}                             n       杈呭姪鍙橀噺
                 * @param   {number}                             m       杈呭姪鍙橀噺
                 * @param   {Array.<number>}                     b       杈呭姪鏁扮粍
                 * @param   {number}                             M       杈呭姪鍙橀噺M
                 * @return  {void}
                 */
                 ...
                 * @param   {Function}                             T    
                 * @param   {Array.<T>}                            t    鎵€姹傛暟缁�
                 * @param   {number}                               n    鍏冪礌涓暟
                 * @return  {System.Collections.Generic.List$1}         鏁扮粍涓璶涓厓绱犵殑缁勫悎鐨勮寖鍨�
                 */
                ...
                CalculateDiPinCai: function (content, methodCode) {
                    var $t;
                    if (methodCode === 100) {
                        return Lottery.BetHelper.GetDingWeiDanCount(content, Lottery.BetHelper.CtDiPinCai10, 3); //瀹氫綅鑳�
                ...
                CalculateKuaiSan: function (content, methodCode) {
                    if (methodCode === 200) {
                        return Lottery.BetHelper.GetKuaiSanDanTuoCount(content); //浜屽悓鍙锋爣鍑�
                    } else if (methodCode === 201) {
                        return Lottery.BetHelper.GetKuaiSanSdCount(content, 2); //浜屽悓鍙锋墜鍔�
                        ...
                        return Lottery.BetHelper.GetKuaiSanBzCount(content, 2); //浜屼笉鍚屾爣鍑�
                    } else if (methodCode === 221) {
                        return Lottery.BetHelper.GetKuaiSanBtSdCount(content, 2); //浜屼笉鍚屾墜鍔�
                    } else if (methodCode === 222) {
                        return Lottery.BetHelper.GetKuaiSanDanTuoCount(content); //浜屼笉鍚岃儐鎷栭€夊彿
                    } else if (methodCode === 300) {
                        return System.Linq.Enumerable.from(System.String.split(content, [44].map(function(i) {{ return String.fromCharCode(i); }}))).count(function (item) {
                                return System.Array.contains(Lottery.BetHelper.CtKuaiSan6, item, System.String);
                            }); //涓夊悓鍙峰崟閫�
                        ...
                        return Lottery.BetHelper.GetKuaiSanBzCount(content, 3); //涓変笉鍚屾爣鍑�
                    } else if (methodCode === 311) {
                        return Lottery.BetHelper.GetKuaiSanBtSdCount(content, 3); //涓変笉鍚屾墜鍔�
                    } else if (methodCode === 600) {
                        return Lottery.BetHelper.GetOneStringCombinationCount(content, 1, Lottery.BetHelper.CtKuaiSanHe); //鍜屽€�
                    }
                ...
                CalculatePkShi: function (content, methodCode) {
                    if (methodCode === 100 || methodCode === 101) {
                        return Lottery.BetHelper.GetDingWeiDanCount(content, Lottery.BetHelper.CtPkShi10, 5); //瀹氫綅鑳�
                    } else if (methodCode === 120) {
                        return Lottery.BetHelper.GetOneStringCombinationCount(content, 1, Lottery.BetHelper.CtPkShi10); //鍓嶄竴
                    } else if (methodCode === 200) {
                        return Lottery.BetHelper.GetFuShiRowGroupDifferentCount(content, 2, Lottery.BetHelper.CtPkShi10); //鍓嶄簩澶嶅紡
                    } else if (methodCode === 201) {
                        return Lottery.BetHelper.GetDanShiRowGroupDifferentCount(content, 2, Lottery.BetHelper.CtPkShi10, 32); //鍓嶄簩鍗曞紡
                    } else if (methodCode === 300) {
                        return Lottery.BetHelper.GetFuShiRowGroupDifferentCount(content, 3, Lottery.BetHelper.CtPkShi10); //鍓嶄笁鍗曞紡 --todo鏃舵椂褰╃粍閫変紭鍖栧埌姝ゆ柟娉�
                    } else if (methodCode === 301) {
                        return Lottery.BetHelper.GetDanShiRowGroupDifferentCount(content, 3, Lottery.BetHelper.CtPkShi10, 32); //鍓嶄笁鍗曞紡
                    } else if (methodCode === 401 || methodCode === 402 || methodCode === 403) {
                        return Lottery.BetHelper.GetOneStringCombinationCount(content, 1, Lottery.BetHelper.ContentDaXiao); //澶у皬
                    } else if (methodCode === 501 || methodCode === 502 || methodCode === 503) {
                        return Lottery.BetHelper.GetOneStringCombinationCount(content, 1, Lottery.BetHelper.ContentDanShuang); //鍗曞弻
                    } else if (methodCode === 601) {
                        return Lottery.BetHelper.GetOneStringCombinationCount(content, 1, Lottery.BetHelper.ContentDaXiaoDanShuang); //鍐犱簹鍜屽ぇ灏忓崟鍙�
                    } else if (methodCode === 602) {
                        return Lottery.BetHelper.GetOneStringCombinationCount(content, 1, Lottery.BetHelper.CtPkShiHe); //鍐犱簹鍜屽拰鍊�
                    }
                ...
                CalculateShiShiCai: function (content, methodCode) {
                    ...
                    if (methodCode === 100) {
                        ...
                        return Lottery.BetHelper.GetRenXuanCount(content, 2); //浠讳簩澶嶅紡
                    } else if (methodCode === 127) {
                        return Lottery.BetHelper.GetRenXuanDanShi(content, 2); //浠讳簩鍗曞紡
                    } else if (methodCode === 128) {
                        return Lottery.BetHelper.GetRenXuanZuXuanCount(content, 2, 2); //浠婚€変簩_缁勯€�
                    } else if (methodCode === 129) {
                        return Lottery.BetHelper.GetRenXuanCount(content, 3); //浠讳笁澶嶅紡
                    } else if (methodCode === 130) {
                        return Lottery.BetHelper.GetRenXuanDanShi(content, 3); //浠讳笁鍗曞紡
                    } else if (methodCode === 131) {
                        return Bridge.Int.mul(Lottery.BetHelper.GetRenXuanZuXuanCount(content, 3, 2), 2); //浠婚€変笁_缁勪笁
                    } else if (methodCode === 132) {
                        return Lottery.BetHelper.GetRenXuanZuXuanCount(content, 3, 3); //浠婚€変笁_缁勫叚
                    } else if (methodCode === 133) {
                        return Lottery.BetHelper.GetRenXuanHunHeCount(content); //浠婚€変笁_娣峰悎缁勯€�
                    } else if (methodCode === 134) {
                        return Lottery.BetHelper.GetRenXuanCount(content, 4); //浠诲洓澶嶅紡
                    } else if (methodCode === 135) {
                        return Lottery.BetHelper.GetRenXuanDanShi(content, 4); //浠诲洓澶嶅紡
                ...
                CalculateShiYiXuanWu: function (content, methodCode) {
                    if (methodCode === 100) {
                        return Lottery.BetHelper.GetDingWeiDanCount(content, Lottery.BetHelper.CtShiYiXuanWu11, 5); //瀹氫綅鑳�
                    } else if (methodCode === 200) {
                        return Lottery.BetHelper.GetFuShiRowGroupDifferentCount(content, 2, Lottery.BetHelper.CtShiYiXuanWu11); //鍓嶄簩鐮乢鐩撮€塤澶嶅紡
                    } else if (methodCode === 201 || methodCode === 211) {
                        return Lottery.BetHelper.GetDanShiRowGroupDifferentCount(content, 2, Lottery.BetHelper.CtShiYiXuanWu11, 32); //鍓嶄簩鐮乢鐩撮€塤鍗曞紡,鍓嶄簩鐮乢缁勯€塤鍗曞紡
                    } else if (methodCode === 210) {
                        return Lottery.BetHelper.GetOneStringCombinationCount(content, 2, Lottery.BetHelper.CtShiYiXuanWu11); //鍓嶄簩鐮乢缁勯€塤澶嶅紡
                    } else if (methodCode === 300) {
                        return Lottery.BetHelper.GetFuShiRowGroupDifferentCount(content, 3, Lottery.BetHelper.CtShiYiXuanWu11); //鍓嶄笁鐮乢鐩撮€塤澶嶅紡
                    } else if (methodCode === 301 || methodCode === 311) {
                        return Lottery.BetHelper.GetDanShiRowGroupDifferentCount(content, 3, Lottery.BetHelper.CtShiYiXuanWu11, 32); //鍓嶄笁鐮乢鐩撮€塤鍗曞紡,鍓嶄笁鐮乢缁勯€塤鍗曞紡
                    } else if (methodCode === 310) {
                        return Lottery.BetHelper.GetOneStringCombinationCount(content, 3, Lottery.BetHelper.CtShiYiXuanWu11); //鍓嶄笁鐮乢缁勯€塤澶嶅紡
                    } else if (methodCode === 900) {
                        return Lottery.BetHelper.GetOneStringCombinationCount(content, 1, Lottery.BetHelper.CtShiYiXuanWu11); //涓嶅畾鑳哶鍓嶄笁
                        ...
                        return Lottery.BetHelper.GetOneStringCombinationCount(content, max.get(methodCode), Lottery.BetHelper.CtShiYiXuanWu11); //浠婚€塤澶嶅紡
                        ...
                        return Lottery.BetHelper.GetDanShiRowGroupOrderDifferentCount(content, max1.get(methodCode), Lottery.BetHelper.CtShiYiXuanWu11, 32); //浠婚€塤鍗曞紡
                    }
