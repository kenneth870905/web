 ...
 * @Autor: kenneth
 * @Date: 2019-08-24 20:36:59
 * @LastEditors: kenneth
 * @LastEditTime: 2019-09-14 18:06:01
 -->
<template>
    <div class="底部">
        <div class="header_1">
            ...
            <i class="icon iconfont icon-hongbao 红色" @click="发布红包()"></i>
            <div>
                您对<span class="红色">大家</span>说：
            </div>
        </div>
        <div class="输入框">
            <div class="textarea_1">
                <div class="">
                    ...
                    <textarea @focus="获得焦点=true" @blur="获得焦点=false" :readonly="!聊天室state.user.IsSend || 聊天室state.房间已禁言" :placeholder="提示" @keyup.enter="keyup($event)" v-model="textarea"></textarea>
                </div>
            </div>
        ...
        <el-dialog title="剪切板图片预览" class="图片预览" :visible.sync="预览图片" width="350px">
    ...
    computed: {
        ...
        提示() {
            if (!this.聊天室state.当前房间.Id) {
                return '您未选择房间，请选择房间'
            } else if (this.聊天室state.房间已禁言) {
                return '房间已禁言'
            } else if (this.聊天室state.user.IsSend) {
                return '请输入内容'
            } else {
                return '当前用户等级已被禁言，请联系客服'
            }
        }
    },
    methods: {
        ...
        发布红包() {
            if (this.聊天室state.user.ChatRole != 1) {
                this.失败提示('权限不足，请联系管理员。');
                return
            }
            this.聊天室state.领取红包.发布弹框 = true;
        },
        选图片() {
            console.log(this.聊天室state.user);
