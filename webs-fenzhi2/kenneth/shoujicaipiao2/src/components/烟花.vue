...
function CreateFireObj() {
    ...
    this.boom = function() {
        // 烟花数量
        var scope = Math.round(getRandom(10, 30));
            scope = 20
        var ww = window.innerWidth
        //var scope = 1;
        for (var i=0; i<scope; i++) {
            var angel = getRandom(0, 2*Math.PI);
            var range = Math.round(getRandom(50, 200)); 	//扩张
            var targetX = ww/2 + range*Math.cos(angel);
            ...
            var color = 'rgb(' + r + ',' + g + ',' + b + ')';
            var frag = new CreateFrag(color, targetX, targetY , ww);
            // console.log(frag)
...
function CreateFrag(color, tx, ty , ww) {
    var that = this
    that.x = ww/2;
    that.y = 200;
    that.ty = ty;
