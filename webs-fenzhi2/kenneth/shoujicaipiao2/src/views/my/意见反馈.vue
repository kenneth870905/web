<template>
    <div>
        <header class="mui-bar mui-bar-nav">
            ...
            <button class="mui-btn mui-btn-blue mui-btn-link mui-pull-right" @click="$router.push('/my/addfeedback')">我要反馈</button>
        </header>
                ...
                <van-list v-model="loading" :finished="query.page>=total && !loading" :finished-text="list.length!=0 ? '没有更多了' : '暂无相关数据'" @load="onLoad">
                    <table class="table">
                        <tr @click="显示详情(item)" v-for="(item, index) in list" :key="index">
                            <td>{{item.Title}}</td>
                            <td :class="item.FeedBackState == 0 ? '未处理':'已处理'">{{item.FeedBackState == 0 ? '未处理':"已处理"}}</td>
        ...
        <van-popup v-model="详情.show" position="bottom" :style="{height:'30%'}">
            <ul class="详情">
                ...
                <li class="" v-if="详情.obj.FeedBackState==0">
                    <label>状态：</label>
                    <div class="未处理">未处理</div>
                </li>
                <template v-else>
                    <li class="">
                        <label>状态：</label>
                        <div class="已处理">已处理</div>
                    </li>
                    <li>
                        <label>人员：</label>
                        <div>{{详情.obj.DealUserName}}</div>
                    </li>
                    <li>
                        <label>回复：</label>
                        <div>
