        ...
        获取所有消息({ state }) {
            return new Promise((resolve, reject) => {
                api_获取所有消息(state.正在聊天code).then(x => {
                    ...
                    state.正在聊天.ReceiverProfile = x.data.ReceiverProfile ? x.data.ReceiverProfile : {}
                    resolve()
                }).catch(err => {
                    reject()
                })
        ...
        async 发送消息({ state, getters, dispatch, commit }, obj) {
            ...
            if (r1.data.Msg == '发送成功' || r1.data.msg == '发送成功') {
                ...
                var obj = {
                    ...
                    SenderCode: state.正在聊天.ReceiverProfile.Code,
                }
