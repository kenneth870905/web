...
export default {
    ...
    actions: {
        ...
        获取所有消息({ state }) {
            return new Promise((resolve, reject) => {
                api_获取所有消息(state.正在聊天code).then(x => {
                    state.正在聊天.list = x.data.MessageInfo.PrivateMessages ? x.data.MessageInfo.PrivateMessages : [];
                    state.正在聊天.SenderProfile = x.data.MessageInfo.SenderProfile ? x.data.MessageInfo.SenderProfile : {}
                    resolve()
                }).catch(err => {
                    reject()
                })
        ...
        async 获取最新消息({ state, dispatch }) {
            ...
            //接收最新消息
            var MessageInfo = x.data.MessageInfo;
            if (MessageInfo) {
                var list = x.data.MessageInfo.PrivateMessages ? x.data.MessageInfo.PrivateMessages : [];
                console.log(list);
