<template>
    <div>
        <div class="room-list" v-if="显示房间" :class="{虚化:显示密码输入框}">
            <!-- <div class="遮罩" @click="显示房间=false"></div> -->
            <div class="遮罩" @click="关闭1()"></div>
            <ul>
                <li @click="选择房间(item)" v-for="(item, index) in 房间列表" :key="index">
                    <span :class="{'active':(item.Id==当前房间.Id && !预设房间) || item.Id==预设房间.Id ,}">
                        {{item.Name}}
                         <i class="sanjiao " ></i>
                         <i class="van-icon van-icon-success" style="font-size:18px; color:#FFF" ></i>
                        <!-- <i> <img src="image/liaotian/icon-xuanzhong.png" style="width: 15px; height: 15px;border-radius: 50%; margin-left: 0px; margin-bottom: 15px;"> </i> -->
                        <b style="font-size: 2.66667vw;
                                    line-height: 3.2vw;
                                    padding: 0 .8vw;
                                    text-align: center;
                                    background: red;
                                    ...
                                    top: -.8vw;
                                    right: -.8vw;
                           border-radius: 3.2vw;" v-if="获取人数(item)">{{获取人数(item)}}人</b>
                    </span>
                </li>
            </ul>
        </div>
        ...
        <div class="mui-content mui-fullscreen" :class="{焦点:焦点}">
            <header class="mui-bar mui-bar-nav"  :style="{background:'url('+config.img_url+'image/liaotian/header.png)'}">
                <a class="mui-action-back mui-icon mui-icon-left-nav mui-pull-left" style="color:#FFF"></a>
                <!-- <a class="mui-icon mui-icon-left-nav mui-pull-left"></a> -->
                <h1 class="mui-title" @click="选中房间()" >
                    <!-- {{到达底部}} -->
                    {{当前房间.Name ? 当前房间.Name :'请选择房间'}}
                    <i :class="{active:显示房间}" class="van-icon van-icon-arrow-down"></i>
                </h1>
                <!-- <a class="mui-icon mui-icon-bars mui-pull-right"></a> -->
                <span @click="$router.push('/my/zxkf')" class="header_icon icon iconfont icon-kefu mui-icon mui-pull-right"></span>
                <!-- <button @click="$router.push('/chatlist')" class="mui-btn mui-btn-blue mui-btn-link mui-pull-right">消息列表</button> -->
                <button @click="$router.push('/chatlist')" class="btn_1 mui-btn mui-btn-blue mui-btn-link mui-pull-right" style="color:#FFF">
                    私信
                    <span v-if="私聊未读>0" class="角标">{{私聊未读}}</span>
                </button>
            </header>

            <div class="开奖列表" :class="{虚化:显示密码输入框}">
                <div class="swiper-container swiper1" ref="轮播">
                    <div class="swiper-wrapper">
                        <div class="swiper-slide" v-for="(item , index) in 开奖列表" :key="index" @tap="$router.push('cpiframe?u=chat&i='+item.key+'&t='+彩票玩法)">
                            <div class="img_box">
                                <img :src="config.img_url+'image/home/caipiao/'+item.key+'.png'" alt="" srcset="">
                            </div>
                            <div class="content_1">
            ...
            <ul class="内容列表" :class="{虚化:显示密码输入框}" ref="内容列表" @scroll="scroll($event)" @click="失去焦点1()">
                <!-- 系统提示 -->
                <li>
                    <xtts1 />
                </li>
            ...
            <!-- <div class="底部" ref="底部输入框"> -->
                ...
                <ul class="键盘">
                    ...
                    <li class="icon_input" ref="icon_input">
                        ...
                        <textarea @keyup.enter="键盘按下($event)" wrap="hard" id="textarea" class="textarea" ref="input_2" :readonly="!聊天室user.IsSend" :placeholder="!聊天室user.IsSend ? '等级不足无法发言，请联系客服':''" v-model="消息文本" type="text" @focus="获取焦点()" @blur="失去焦点()"></textarea>
                ...
                <ul class="列表1" v-if="显示自定义添加">
                    <li  @click="选择图片()">
                        <div><i> <img src="image/liaotian/icon-image.png" style="width:40px;height:40px;margin-top:10px;" alt=""> </i></div>
                        <div>照片</div>
                    </li>
                    <li @click="$toast('暂未开放')">
                        <div><i><img src="image/liaotian/icon-red-envelope.jpg" style="width:40px;height:40px;margin-top:10px;"></i></div>
                        <div>红包</div>
                    </li>
                    <li class="换行"></li>
                    <li @click="显示选择彩票=true">
                        <div>
                            <i><img src="image/liaotian/icon-cai.png" style="width:40px;height:40px;margin-top:8px;"></i>
                        </div>
                        <div>彩票</div>
                    </li>
                    <li @click="$router.push('/tzjilu?back=1&state=1')">
                        <div>
                            <i> <img src="image/liaotian/icon-zhudan.png" style="width:40px;height:40px;margin-top:8px;border-radius: 50%;"> </i>
                        </div>
                        <div>注单</div>
                    </li>
                    <li @click="$router.push('/chongzhi')">
                        <div>
                            <i > <img src="image/liaotian/icon-chongzhi.jpg" style="width:32px;height:32px;margin-top:12px;">  </i>
                        </div>
                        <div>充值</div>
                    </li>
                    <li @click="$router.push('/tixian')">
                        <div>
                            <i>  <img src="image/liaotian/icon-tixian.png" style="width:32px;height:32px;margin-top:12px;"> </i>
                        </div>
                        <div>提现</div>
                    </li>
                    <!-- <li @click="$toast('暂未开放')">
                ...
                <div class="表情" v-if="显示表情包.show">
                    <div class="content_1">
                        ...
                        <ul class="表情包2" v-if="显示表情包.type==1">
                            <li v-for="(item, index) in 表情列表1" :key="index">
                                <img :src="config.img_url+'image/liaotian/'+item" alt="">
                            </li>
                        </ul>
                    ...
                    <ul class="表情选项">
                        <li @click="显示表情包.type=0"><img :src="config.img_url+'image/liaotian/emoji-0.png'" alt="" srcset=""></li>
        ...
        <div class="密码输入" v-show="显示密码输入框" >
            <div class="内容" ref="密码输入框">
                ...
                <i @click="设置key(['显示密码输入框',false])" class="van-icon van-icon-cross 关闭"></i>
                <div class="input_box">
...
// });
// 开奖号码
const yq = () => import('@/pages/开奖/components/开奖号码/圆球号码.vue')
const pk10 = () =>import('@/pages/开奖/components/开奖号码/pk10号码.vue')
const lhc = () =>import('@/pages/开奖/components/开奖号码/六合彩号码.vue')
const erba = () =>import('@/pages/开奖/components/开奖号码/28.vue')
...
export default {
    ...
    data() {
        return {
            ...
            // config.api_url + "/Scripts/signalR/jquery.signalR-2.2.2.js"
            js1: "",
            // config.api_url + "/signalr/hubs"
            js2: "",
            // config.api_url + "/Areas/MApi/Content/chat.js"
            js3: "",
            密码输入: {
                show: false,
                房间密码: '',
    ...
    methods: {
        ...
        获取人数(item){
            var obj=''
            try {
                obj=this.会员人数.find(x=>x.Id==item.Id); 
            } catch (error) {}
            if(obj){
        ...
        loadRongJs2() {
            this.js3 = config.api_url + "/Areas/MApi/Content/chat.js?v=1"
        },
        loadRongJs3() {
            console.log('js 加载完成');
            this.登录聊天室();
        ...
        设置轮播() {
            var this_1=this;
            var index=0
            if(this.偏好彩票 <= Object.keys(this_1.开奖列表).length*2){
                index=this.偏好彩票
            }
                console.log('偏好彩票',this.偏好彩票)
        ...
        async 进入聊天室() {
            if (typeof (appPage) != 'object') {
                await this.获取远程js(config.api_url + "/Scripts/signalR/jquery.signalR-2.2.2.js?v=1")
                await this.获取远程js(config.api_url + "/signalr/hubs")
                await this.获取远程js(config.api_url + "/Areas/MApi/Content/chat.js?v=1")
                console.log('获取js成功')   
                this.登录聊天室() 
            } else {
                console.log('再次进入聊天室')
...
.sanjiao{
    ...
    border-color: #24cf0e transparent transparent transparent;
    top:0;
    position: absolute;
}
.sanjiao i{
...
.room-list ul span.active{
    color: yellow !important;
}
.room-list ul span{
      color: white;
      background: linear-gradient(to right, #ff4b4b, #9373ff) !important;
}
.btn_1{
    padding: 0px 5px;
    position: relative;
