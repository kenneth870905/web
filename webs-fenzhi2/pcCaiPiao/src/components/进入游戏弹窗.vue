    ...
    <div>
        <el-dialog :title="待进入游戏.name" width="400px" :visible.sync="显示进入游戏弹框" :before-close="handleClose">
            <el-form :model="user" ref="user">
                ...
                <el-form-item>
                    <div class="item">
                        <el-input class="input_1" :value="userInfo.Money" size="small" :loading="true" disabled>
                            <template slot="prepend">平台余额</template>
                        </el-input>
                        <el-button size="small" @click="getUserInfo()" type="primary" :disabled="userInfo.UserId ? false : true">刷新</el-button>
                    </div>
                </el-form-item>
                <el-form-item>
                    <div class="item">
                        <el-input class="input_1" :value="MinMoney" size="small" :loading="true" disabled>
...
import { api_转入金额 } from '@/api/余额接口.js'
    ...
    methods: {
        ...
        submitForm(formName) {
            if (this.userInfo.UserId) {
                ...
                var url = config.iframe_url + '/M/Other/Go?gameId=' + this.待进入游戏.gid + '&tryOut=&type=' + this.待进入游戏.查询类型
                window.open(url)
            } else {
