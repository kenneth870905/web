<template>
    <div class="彩票">
        <div class="类型选择 定位" > 
            <div class="item 全部" :style="{'background':皮肤区分.topbj,'color':皮肤区分.color}"  @click="点击类型('',0)" :class="{'active':选中彩票类型==''}">
                <span>热门彩票</span>    
            </div>
            <div class="swiper-container swiper_1" ref="swiper">
                <div class="swiper-wrapper">
                    <div class="swiper-slide item" :style="{'background':皮肤区分.topbj,'color':皮肤区分.color}"  v-if="item.title!='所有彩种'" @click="点击类型(item,index)" :class="{'active':选中彩票类型==item}" v-for="(item, index) in 彩种类型" :key="index">
...
.类型选择{
 ...
 box-shadow: 1px 0 3px rgba(0,0,0,.16);
    ...
    .全部{
         border-right:_vw(1)  solid  rgba(0,0,0,.16);
          box-shadow: -20px -2px -15px -13px   rgba(0,0,0,.16) inset;
    }
    .item{
        display: flex;
        align-items: center;
